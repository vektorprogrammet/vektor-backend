...
class CsvUtil
{
    ...
    public static function csvFromTable($columnMap, $rows, string $sep = ';'): string
    {
        ...
        foreach ($columnMap as $question) {
            $content .= self::csvEscapeAndSeparate($question, $sep);
        }
        $content = self::csvNewline($content);
        foreach ($rows as $csv_row) {
            foreach ($columnMap as $id => $qname) {
                if (isset($csv_row[$id])) {
                    $content .= self::csvEscapeAndSeparate($csv_row[$id], $sep);
                } else {
                    $content .= self::csvEscapeAndSeparate('', $sep);
                }
            }
            $content = self::csvNewline($content);
        }
