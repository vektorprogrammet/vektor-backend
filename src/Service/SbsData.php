...
class SbsData extends ApplicationData
{
    ...
    {
        return (int) (ceil(($this->getAdmissionPeriod()->getEndDate()->getTimestamp() - (new \DateTime())->getTimestamp()) / 3600));
    }

    public function getTimeToAdmissionStart(): int
    {
        return (int) (ceil(($this->getAdmissionPeriod()->getStartDate()->getTimestamp() - (new \DateTime())->getTimestamp()) / 3600));
    }
    ...
    private function determineCurrentStep(AdmissionPeriod $admissionPeriod, $interviewedAssistantsCount, $assignedInterviewsCount, $totalAssistantsCount): float
    {
        ...
        if (0 === $assignedInterviewsCount && 0 === $interviewedAssistantsCount) {
            return 3;
