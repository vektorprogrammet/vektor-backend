...
class TeamPositionSortExtensionUnitTest extends TestCase
{
    ...
    public function testExecutiveMembers()
    {
        ...
        for ($x = 0; $x < 7; ++$x) {
            $this->assertSame($sortedMemberships[$x]->getActiveExecutiveBoardMemberships()[0]->getPositionName(), $sortedPositions[$x]);
        }
    }
    ...
    public function testTeamMemberships()
    {
        ...
        $sortedPositions = ['Leder', 'Nestleder', 'Assistent', 'Medlem', 'Sekretær', 'Økonomi', ''];
        for ($x = 0; $x < 7; ++$x) {
            $this->assertSame($sortedMemberships[$x]->getActiveTeamMemberships()[0]->getPositionName(), $sortedPositions[$x]);
        }
    }
}
